...
def get_mono_vac_energy(atoms=None, id=""):
    strts = Vacancy(atoms).generate_defects(
        on_conventional_cell=True, enforce_c_size=5, extend=1
        #on_conventional_cell=True, enforce_c_size=10, extend=0
    )
    ...
    for j in strts:
        ...
        print(' Vacancy name',name, j.to_dict()["symbol"])
        def_en = info_def["energy"] - epa * perf_atoms.num_atoms + chempot[j.to_dict()["symbol"]]
        info={}
        info['name']=name
        info['def_en']=def_en
        info['epa']=def_en
...
print("mem", len(mem))
dumpjson(data=mem, filename="mono_vac2multi.json")
